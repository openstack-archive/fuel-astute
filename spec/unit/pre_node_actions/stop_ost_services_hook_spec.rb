...
describe Astute::StopOSTServices do
  ...
  it 'should upload "stop_services.script" from correct source' do
    #File.expects(:read).with(regexp_matches(/stop_services.script/)).returns("script")
    stop_ost_services.expects(:upload_script).with(ctx, deploy_data.map{ |n| n['uid'] }, regexp_matches(/PreDeploy/)).once
    stop_ost_services.expects(:run_shell_command).once.returns(:data => {:exit_code => 0})
    stop_ost_services.process(deploy_data, ctx)
  end
