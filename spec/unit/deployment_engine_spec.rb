...
describe Astute::DeploymentEngine do
  ...
  describe '#deploy' do
    ...
    it 'should skip failed nodes' do
      ...
      ctx.expects(:report_and_update_status).with('nodes' => [{'uid' => '3', 'status' => 'error', 'error_type' => 'deploy', 'role' => 'deploy', 'error_msg' => 'Node is not ready for deployment'}], 'error' => 'Critical nodes are not available for deployment')
    ...
    it 'should raise error if critical node is missing' do
      ...
      ctx.expects(:report_and_update_status).with('nodes' => [{'uid' => '2', 'status' => 'error', 'error_type' => 'deploy', 'role' => 'deploy', 'error_msg' => 'Node is not ready for deployment'}], 'error' => 'Critical nodes are not available for deployment')
