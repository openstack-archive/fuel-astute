...
describe Astute::NailgunHooks do
  ...
  context "#process" do
    it 'should process known hook type' do
      ...
      hooks.expects(:upload_file_hook).returns({'success' => true, 'error' => nil})
      hooks.expects(:puppet_hook).returns({'success' => true, 'error' => nil})
      hooks.expects(:shell_hook).returns({'success' => true, 'error' => nil})
      hooks.expects(:sync_hook).returns({'success' => true, 'error' => nil})
      hooks.expects(:reboot_hook).returns({'success' => true, 'error' => nil})
      hooks.expects(:copy_files_hook).returns({'success' => true, 'error' => nil})
    ...
    it 'should run hooks by priority order' do
      ...
      hooks.expects(:upload_file_hook).returns({'success' => true, 'error' => nil}).in_sequence(hook_order)
      hooks.expects(:copy_files_hook).returns({'success' => true, 'error' => nil}).in_sequence(hook_order)
      hooks.expects(:shell_hook).returns({'success' => true, 'error' => nil}).in_sequence(hook_order)
      hooks.expects(:sync_hook).returns({'success' => true, 'error' => nil}).in_sequence(hook_order)
      hooks.expects(:puppet_hook).returns({'success' => true, 'error' => nil}).in_sequence(hook_order)
      hooks.expects(:reboot_hook).returns({'success' => true, 'error' => nil}).in_sequence(hook_order)
    ...
    context 'critical hook' do
      ...
      it 'should raise exception if critical hook failed' do
        ...
        hooks.expects(:copy_files_hook).returns({'success' => 'true', 'error' => nil})
        hooks.expects(:upload_file_hook).returns({'success' => 'true', 'error' => nil})
        hooks.expects(:shell_hook).returns({'success' => false, 'error' => 'Shell error'})
      ...
      it 'should process next hooks if non critical hook failed' do
        hooks = Astute::NailgunHooks.new(hooks_data, ctx)
        hooks.expects(:upload_file_hook).returns({'success' => false, 'error' => 'Upload error'})
        hooks.expects(:shell_hook).returns({'success' => true, 'error' => nil})
          hooks.expects(:sync_hook).returns({'success' => false, 'error' => 'Sync error'})
        hooks.expects(:puppet_hook).returns({'success' => true, 'error' => nil})
        hooks.expects(:reboot_hook).returns({'success' => true, 'error' => nil})
          ...
          {'nodes' =>
            [
              { 'uid' => '1',
                ...
                'error' => "Puppet run failed. Check puppet logs for details"
              },
              { 'uid' => '3',
                'status' => 'error',
                'error_type' => 'deploy',
