...
describe Astute::NailgunHooks do
  ...
  let(:upload_file_hook) do
    {
      "priority" =>  100,
      "type" =>  "upload_file",
      "fail_deployment_on_error" => false,
  ...
  context "#process" do
    ...
    context 'critical hook' do
      ...
      before(:each) do
        hooks_data[2]['fail_deployment_on_error'] = true
  ...
  context '#shell_hook' do
    ...
    context 'process data from mcagent in case of critical hook' do
      before(:each) do
        shell_hook['fail_deployment_on_error'] = true
        ctx.stubs(:report_and_update_status)
      end
  ...
  context '#upload_file_hook' do
    ...
    context 'process data from mcagent in case of critical hook' do
      before(:each) do
        upload_file_hook['fail_deployment_on_error'] = true
        ctx.stubs(:report_and_update_status)
      end
  ...
  context '#sync_hook' do
    ...
    context 'process data from mcagent in case of critical hook' do
      before(:each) do
        sync_hook['fail_deployment_on_error'] = true
        ctx.stubs(:report_and_update_status)
      end
  ...
  context '#puppet_hook' do
    ...
    context 'process data from mcagent in case of critical hook' do
      before(:each) do
        puppet_hook['fail_deployment_on_error'] = true
        ctx.stubs(:report_and_update_status)
      end
