...
describe "NailyFact DeploymentEngine" do
  ...
  context "When deploy is called, " do
    ...
    before(:each) do
      ...
      deploy_engine.stubs(:generate_and_upload_ssh_keys).with(uniq_nodes_uid, deploy_data.first['deployment_id'])
      deploy_engine.stubs(:sync_puppet_manifests).with(deploy_data)
    end
