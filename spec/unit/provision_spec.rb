...
describe Astute::Provisioner do
  ...
  describe '#provision' do
    ...
    context 'node state cases' do
      ...
      it "fail if timeout of provisioning is exceeded" do
        @provisioner.stubs(:provision_and_watch_progress).returns([[],[{'uid' => '1'}]])
  ...
  describe '#provision_and_watch_progress' do
    ...
    it "raises error if nodes list is empty" do
      expect {@provisioner.provision_and_watch_progress(@reporter, data['task_uuid'], {}, data['engine'], 0)}.
                            to raise_error(/Nodes to provision are not provided!/)
    end
    ...
    it "raise error if failed node find" do
      expect do
        ...
        @provisioner.provision_and_watch_progress(@reporter, data['task_uuid'], data['nodes'], data['engine'], 0)
    ...
    it "unexpecting bootstrap nodes should be ereased and rebooted" do
      ...
      @provisioner.provision_and_watch_progress(@reporter, data['task_uuid'], nodes, data['engine'], 0)
    end
    ...
    it 'should success if only one node fails' do
      ...
      Astute.config.max_prc_to_fail = 40 #in percents
      nodes = [
        { 'uid' => '1'},
        { 'uid' => '2'},
        { 'uid' => '3'}
    ...
    it 'should fail if one node fails' do
      ...
      Astute.config.provisioning_timeout = 5
      Astute.config.max_nodes_per_call = 2
      Astute.config.max_prc_to_fail = 0 #in percents
      nodes = [
        { 'uid' => '1'},
        { 'uid' => '2'},
        { 'uid' => '3'}
