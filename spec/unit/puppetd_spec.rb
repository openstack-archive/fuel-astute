...
describe "Puppetd" do
  ...
  context "PuppetdDeployer" do
    ...
    context "puppet state transitions" do
      ...
      it "publishes error status for node if puppet runing alien task (attempts been exhausted)" do
      ...
      it "ignore exit code of puppet runing of alien task (waited for alien task stop and launched own)" do
        rpcclient.stubs(:last_run_summary).at_least(3).
          returns([ mock_mc_result(last_run_result_running) ]).then.
          returns([ mock_mc_result(last_run_result_running) ]).then.
        ...
        rpcclient.expects(:runonce).once.returns([ mock_mc_result(last_run_result) ])
        reporter.expects(:report).with('nodes' => [{'uid' => '1', 'status' => 'ready', 'progress' => 100, 'role' => 'compute'}])
