  ...
  module Agent
    class Erase_node < RPC::Agent
      ...
      def reboot
        ...
        Log.debug("Run node rebooting command '#{cmd}'")
        pid = fork { system(cmd) }
        Process.detach(pid)
      end
      ...
      def erase_data(dev, length=1, offset=0)
        ...
        status = system(cmd)
        Log.debug("Run device erasing command '#{cmd}' returned '#{status}'")
      ...
      def prevent_discover
        ...
        Log.debug("Create file for discovery preventing #{lock_path}")
