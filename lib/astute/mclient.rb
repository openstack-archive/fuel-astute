...
module Astute
  ...
  private
    ...
    def check_results_with_retries(method, args)
      ...
      if err_msg.present?
        Astute.logger.error
        expired_size = failed.select { |n| n.results[:statusmsg] == 'execution expired' }.size
        # Detect TimeOut: 1 condition - fail because of custom timeout, 2 - fail because of DDL timeout
        if (failed.present? && failed.size == expired_size) || (timeout_nodes_count > 0 && failed.empty?)
          raise Timeout::Error, "#{@task_id}: #{err_msg}"
        else
