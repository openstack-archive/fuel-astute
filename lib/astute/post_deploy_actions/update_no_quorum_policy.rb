...
module Astute
  class UpdateNoQuorumPolicy < PostDeployAction
    ...
    def process(deployment_info, context)
      ...
      primary_controller = deployment_info.first['nodes'].inject([]) do |c_n, n|
        c_n << n['uid'] if ['primary-controller'].include? n['role']
        c_n
      end

      response = run_shell_command(context, primary_controller, cmd)
