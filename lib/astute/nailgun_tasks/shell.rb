...
module Astute
  class Shell < NailgunTask
    ...
    def setup_default
      ...
      @hook['parameters']['cwd'] ||= "/"
      @hook['parameters']['retries'] ||= Astute.config.mc_retries
      @hook['parameters']['interval'] ||= Astute.config.mc_retry_interval
    end
    ...
    def shell_exec_template
      command = "cd #{@hook['parameters']['cwd']} &&" \
                " #{@hook['parameters']['cmd']}"
      template = <<-eos
    #!/bin/bash
    # Puppet shell wrapper for task: <%= task_name %>
    # Manifest: <%= puppet_exec_file_path task_name %>
