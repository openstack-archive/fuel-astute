...
module Astute
  class PuppetTask
    ...
    def initialize(ctx, node, options={})
      ...
      @puppet_mclient = PuppetMClient.new(@ctx, @node['uid'], @options)
    end
    ...
    def puppet_status
      ...
      if UNDENINED_STATUSES.include?(actual_status)
    ...
    def puppet_to_task_status(mco_puppet_status)
      case
      when SUCCEED_STATUSES.inlcude?(mco_puppet_status)
        'successful'
      when BUSY_STATUSES.include?(mco_puppet_status)
        'running'
      when STOPPED_STATUSES.include?(mco_puppet_status)
        'failed'
      when UNDENINED_STATUSES.include?(mco_puppet_status)
        'failed'
      else
