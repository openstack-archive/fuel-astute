  ...
  class SshEraseNodes
    ...
    def self.command
      <<-ERASE_COMMAND
        ...
        BLOCK_DEVICES=$(sed -nr 's/^.*[0-9]\s+([a-z]+)$/\\1/p' /proc/partitions)
	  ...
	  blockdev --flushbufs "/dev/$1"
        ...
        erase_partitions() {
          for PART in $(sed -nr 's/^.*[0-9]\s+('"$1"'[0-9]+)$/\\1/p' /proc/partitions)
          do
        ...
        erase_boot_devices() {
          ...
          do
            ...
            echo "$STORAGE_CODES" | grep -wq "$MAJOR" || continue
            grep -wq 0 "/sys/block/$DEVICE/removable" || continue
        ...
        if [ -r /etc/nailgun_systemtype ]
          then NODE_TYPE=$(cat /etc/nailgun_systemtype)
          else NODE_TYPE="provisioning"
        fi

        if grep -Eq 'root=[^[:blank:]]+' /proc/cmdline
          then
