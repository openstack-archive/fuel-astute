...
module Astute
  class TaskDeployment
    ...
    def deploy(deployment_info, deployment_tasks)
      ...
      cluster = Deployment::Cluster.new # cluster_id?
      ...
      deployment_tasks.each do |node_id, tasks|
        tasks.each do |task|
          task['requires'].each do |d_t|
            cluster[node_id][task['id']].depends nodes[d_t['node_id']][d_t['name']]
          end

          task['required_for'].each do |d_t|
            cluster[node_id][task['id']].depended_on nodes[d_t['node_id']][d_t['name']]
          end
        end
      end
