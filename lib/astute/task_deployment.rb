...
module Astute
  ...
  class GraphNode < Deployment::Node
    def context=(context)
      @ctx = context
    end
  ...
  class TaskDeployment
    ...
    def deploy(deployment_info, deployment_tasks)
      ...
      # FIXME: Add additional gem and package dependences into
      # Astute container before run this part of code
      # For testing propose
      # Short instruction:
      #   dockerctl shell astute
      #   gem install ruby-graphviz
      #   yum install graphviz
      plot_deployment = Deployment::PlotProcess.new(nodes.values)
      plot_deployment.gv_load
      File.open("graph-#{@ctx.task_id}.dot", 'w') do |f|
        f.write(plot_deployment.to_dot)
      end
