  ...
  module RaidVendors
    class Lsi
      ...
      def delete
        cmd = case @interface
        when
          "#{EXEC} /c#{@controller_id}/v#{@vitrual_device} del J"
        when 'api'
          "#{CURL} -X DELETE http://127.0.0.1:8080/v0.5/controllers/#{@controller_id}/virtualdevices/#{@vitrual_device}"
        end
