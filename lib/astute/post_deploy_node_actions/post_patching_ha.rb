...
module Astute
  class PostPatchingHa < PostDeployNodeAction

    def process(deployment_info, context)
      old_env=deployment_info.first['openstack_version_prev']
      return if old_env.nil? and
          deployment_info.first['nodes']['deployment_mode'] !~ /ha/i
      ...
      pacemaker_services_list.each do
        ...
        if deployment_info.select {|n|
          ...
          pcmk_ban_cmd="crm resource start #{pacemaker_service} && sleep 3"
        else
          pcmk_ban_cmd="pcs resource clear #{pacemaker_service} `crm_node -n` && sleep 3"
        end
        Astute.logger.info "Unbanning pacemaker service #{pacemaker_service}"
    ...
    def get_pacemaker_services_list(deployment_info)
      ...
      if deployment_info.first['nodes']['quantum'].to_s == 'true'
      ...
      if deployment_info.first['nodes']['ceilometer']['enabled'].to_s == 'true'
