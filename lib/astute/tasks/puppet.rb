  ...
  class Puppet < Task
    ...
    def setup_default
      default_options = {
        ...
        'fade_timeout' => Astute.config.puppet_fade_timeout
      }
      @task['parameters'].compact.reverse_merge!(default_options)
    end

    def puppet_task
      @puppet_task ||= PuppetJob.new(
        task_name,
        Astute::PuppetMClient.new(@ctx, @task['node_id'], @task['parameters']),
        {
