...
module Astute
  ...
  class Orchestrator
    ...
    def stop_provision(reporter, task_id, engine_attrs, nodes)
      ...
      target_uids, _nodes_not_booted = analize_node_types(nodes_types, nodes.map { |n| n['uid'] })

      provisioned_nodes = target_uids.map { |uid| {'uid' => uid } }
      ...
      # Remove possible duplication: same node uids in success and in error/inaccessible state
      result['nodes'] -= result['error_nodes']
      result['nodes'] -= result['inaccessible_nodes']
