...
module Astute
  ...
  class Orchestrator
    ...
    def validate_nodes_access(ctx, nodes)
      # Do not check master node availability, because it does not contain
      # node type file
      node_ids = nodes.map{ |n| n['uid'].to_s } - ['master']
      nodes_types = node_type(ctx.reporter, ctx.task_id, node_ids, timeout=10)
      not_avaliable_nodes = node_ids - nodes_types.map { |n| n['uid'].to_s }
