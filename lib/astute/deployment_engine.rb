...
module Astute
  class DeploymentEngine
    ...
    def remove_failed_nodes(deployment_info, pre_deployment, post_deployment)
      ...
      available_nodes = systemtype.get_type().select { |node| node[:data][:node_type] == "target"}
      available_uids = available_nodes.map { |node| node[:sender]}

      deployement_info = deployement_info.select { |node| available_uids.include? node['uids'][0] }
