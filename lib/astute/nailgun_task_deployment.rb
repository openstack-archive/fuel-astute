...
module Astute
  ...
  class NailgunNode < Deployment::Node
    ...
    def current_progress_bar
      100 * self.tasks_finished_count / self.tasks_total_count
    end
  ...
  class NailgunTaskDeployment
    ...
    def deploy(deployment_info, deployment_tasks)
      ...
      nodes = deployment_tasks.keys.inject([]) do |nodes, node_id|
        ...
        node.context = @ctx
        #TODO: add support for failed or offile node
        #TODO: add mark nodes as critical
        nodes << node
      end

      deployment = Deployment::Process.new(nodes)
      deployment.run
    end
