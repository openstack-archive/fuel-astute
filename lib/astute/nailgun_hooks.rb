...
module Astute
  class NailgunHooks
    ...
    def copy_files_hook(hook)
      ...
      validate_presence(hook['parameters'], 'srcs_and_dests')

      is_success = true
      hook['parameters']['srcs_and_dests'].each do |src, desc|
        hook['parameters']['content'] = File.read(src)
        hook['parameters']['path'] = desc
