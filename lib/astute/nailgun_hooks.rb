  ...
  class NailgunHooks
    ...
    def shell_hook(hook)
      ...
      perform_with_limit(hook['uids']) do |node_uids|
        Timeout::timeout(timeout) do
          ...
          ret['error']= "Failed to run command #{shell_command}" if !succeed
        end
      end
    ...
    def sync_hook(hook)
      ...
      perform_with_limit(hook['uids']) do |node_uids|
        ...
        ret = {'error' => "Failed to perform sync from #{source} to #{path}"} if !succeed
      end
    ...
    def run_shell_command(context, node_uids, cmd, retries, interval, timeout=60, cwd="/tmp")
      retries.times.each do |retry_number|
        Astute.logger.error "retry_number #{retry_number}"
        ...
        Astute.logger.warn "Problem while performing cmd on nodes: #{node_uids}. "
          "Try to repeat: #{retry_number} attempt"
        sleep interval
      end
