...
module Astute
  module Network
    ...
    def self.network_flow(ctx, nodes, task)
      _flow = [self.start_action, self.send_action, self.get_info_action]
      data = {'config' => nodes,
              'check' => task}
      result = {'progress'=>0}
      # TODO Everything breakes if agent not found. We have to handle that
      agent = MClient.new(ctx, "net_probe", nodes.keys)
      for action in _flow
        response = action(ctx, agent, data)
        ...
        if nodes_status.include? 'error'
          result['status'] = 'error'
          self.clean_action(ctx, agent, data)
          break
        elsif nodes_status.all? {|status| status == 'success'}
    ...
    def self.start_action(ctx, agent, data)
      data['command'] = 'start'
      net_probe.check(:data=>data.to_json)
    end
    ...
    def self.format_response(response)
      ...
      response.each do |node|
        formatted[node[:sender]] = {'status'=>node.results[:status],
