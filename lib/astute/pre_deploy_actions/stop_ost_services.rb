...
module Astute
  class StopOSTServices < PreDeployAction
    ...
    def getstop_cmd(deployment_info, services_list)
      ...
      result = ""
      # 1st, try a gracefull service stop way
      result += "service --status-all 2>&1 | egrep '#{services_list}' | awk '/#{delimiter}/ {print #{pos}}' |"
      result += "xargs -n1 -I {} echo service {} stop >/dev/null 2>&1;"
      # 2nd, check for running processess, if any left
      ...
      result += "perl -e \"while(<>){ push @r, /\((\d+)\)/sg }; print join(' ',@r)\");"
      # and send SIGKILL
      result += "for pid in `echo $pids_to_kill`; do echo kill -9 $pid; done; done"
      return result
    end
  end #class
end
