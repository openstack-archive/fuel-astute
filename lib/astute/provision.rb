...
module Astute
  ...
  class Provisioner
    ...
    def provision(reporter, task_id, provisioning_info, provision_method)
      ...
      result_msg = {'nodes' => []}
      Astute.logger.info("Provision method: #{provision_method}")
      begin
        if provision_method == 'native'
          check_ubuntu_repo_connectivity(nodes, task_id, reporter, fault_tolerance)
        end
    ...
    def check_ubuntu_repo_connectivity(nodes, task_id, reporter, fault_tolerance)
      ...
      Astute.logger.debug("Checking repo connectivity for nodes #{node_ids.inspect}")
      ...
      Astute.logger.debug("Repo connectivity check result: #{result}")

      failed_ids = result.select{|n| n['exit code'] != 0}.map{|n| n['uid']}

      if should_fail(failed_ids, fault_tolerance)
