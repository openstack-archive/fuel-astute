...
module Astute
  ...
  class Provisioner
    ...
    def provision(reporter, task_id, engine_attrs, nodes)
      ...
      max_nodes_to_fail = Astute.config[:max_nodes_to_fail] / 100.0 * nodes.count
      ...
      begin
        ...
        failed_nodes, timeouted_nodes = watch_provision_progress(reporter, task_id, Array.new(nodes), engine_attrs, max_nodes_to_fail)

        failed_uids = failed_nodes.map { |n| n['uid'] }
        timeouted_uids = timeouted_nodes.map { |n| n['uid'] }
      rescue => e
      ...
      if failed_uids.present?
        Astute.logger.error("Provision of some nodes failed. Failed nodes: #{failed_nodes}")
      ...
      if failed_uids.count + timeouted_uids.count > max_nodes_to_fail
        result_msg['status'] = 'error'
        result_msg['error'] = 'To many nodes failed to provision'
    ...
    def watch_provision_progress(reporter, task_id, nodes_to_provision, engine_attrs, max_nodes_to_fail)
        ...
        loop do
          sleep_not_greater_than(20) do
            ...
            if nodes_not_booted.count < max_nodes && nodes_to_provision.count > 0
              ...
              failed_nodes += provision_piece(reporter, task_id, engine_attrs, new_nodes)
            ...
            if failed_nodes.count + timeouted_nodes.count > max_nodes_to_fail
              Astute.logger.debug("Aborting provision. To many nodes failed: #{failed_nodes + timeouted_nodes}")
              return failed_nodes, timeouted_nodes
            end
