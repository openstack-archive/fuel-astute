...
class Astute::DeploymentEngine::Tasklib < Astute::DeploymentEngine
  ...
  def run_task(node_id, task)
    Astute.logger.info "#{@ctx.task_id}: run task '#{task}' for node uid=#{node_id}"
    cmd = "taskcmd daemon #{task}"
      ...
      @task_manager.node_uids.each do |node_id|
        if task = @task_manager.current_task(node_id)
          ...
          when :running
            ...
            # TODO(vsharshov): return old log parsing temporary
          when :valid_but_failed then nodes_to_report << process_fail_node(node_id, task)
          when :unexpected_error then nodes_to_report << process_fail_node(node_id, task)
          when :not_found_such_task then nodes_to_report << process_fail_node(node_id, task)
          else
