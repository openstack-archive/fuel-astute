...
<% end -%>
    ...
    def make_image(options={})
      ...
      type = options.fetch :type, dot_type
      size = options.fetch :size, dot_size
      ...
      if size
        size = [size] unless size.is_a? Array
        command += size if size
      end
    ...
    def count_tolerance_fail(node)
      return unless @fault_tolerance_groups.is_a? Array
      info "Count failed node: #{node.name}"
    ...
    def fault_tolerance_excess?
      return false unless @fault_tolerance_groups.is_a? Array
